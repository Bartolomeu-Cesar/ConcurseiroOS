...
def create_simulado(body: SimuladoCreate, conn=Depends(get_db_session)):
    cur = conn.execute("""
...
def responder_simulado(id: int, body: SimuladoResponder, conn=Depends(get_db_session)):
...
def finalizar_simulado(id: int, body: SimuladoFinalizar, conn=Depends(get_db_session)):
...
def delete_simulado(id: int, conn=Depends(get_db_session)):
...
def simulado_prova_real(body: SimuladoProvaReal, conn=Depends(get_db_session)):
...
def simulado_inteligente(qtd: int = 10, conn=Depends(get_db_session)):
...
def simulado_adaptativo(materia: str = "", qtd: int = 10, conn=Depends(get_db_session)):
