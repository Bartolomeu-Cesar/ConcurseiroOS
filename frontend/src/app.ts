...
function toast(message: string, type: string = 'info', duration: number = 4000, action: ToastAction | null = null): HTMLElement {
...
// ==================== LOADING STATES ====================
function showLoading(container: string | HTMLElement): void {
...
function showSpinner(container: string | HTMLElement): void {
  if (typeof container === 'string') container = document.getElementById(container);
  if (!container) return;
  container.innerHTML = '<div class="spinner-wrapper"><div class="spinner"></div></div>';
}

function showEmpty(container: string | HTMLElement, icon: string = '📚', message: string = 'Nenhum item encontrado'): void {
  if (typeof container === 'string') container = document.getElementById(container);
  if (!container) return;
...
async function api(url: string, options: ApiOptions = {}): Promise<any> {
...
function quickSearchQuery(query: string): void {
...
function goToEditalItem(id: number): void {
  closeActiveOverlay();
...
function undoableDelete(itemDesc: string, deleteUrl: string, onComplete?: (deleted: boolean) => void): void {
...
var editalData: EditalItem[] = []; // Declaração global (usada por PDFs e Edital)
...
// --- Pomodoro Timer ---
let timerInterval: number | null = null, elapsed: number = 0, paused: boolean = false, limitSeconds: number = 0, startedAt: number | null = null;
...
// ==================== TAB 2: EDITAL ====================
let editalTimer: number | null = null, editalStartedAt: number | null = null, editalElapsed: number = 0, editalPaused: boolean = false, editalSelectedId: number | null = null;
...
// ==================== TAB 3: CICLO ====================
let cicloTimerInterval: number | null = null, cicloStartedAt: number | null = null, cicloElapsed: number = 0, cicloPaused: boolean = false;
let cicloProximoId: number | null = null, cicloProximoMateria: string = '';
...
// ==================== TAB 4: FLASHCARDS ====================
let flashcardsToday: Flashcard[] = [], currentFlashIndex: number = 0;
...
async function reviewFlashcard(quality: number): Promise<void> {
  const card = flashcardsToday[currentFlashIndex];
  if (!card) return;
  try {
...
// ==================== NOTAS POR TÓPICO ====================
let noteCurrentId: number | null = null;
...
let selectModalCallback: ((item: SelectItem) => void) | null = null;

function openSelectModal(title: string, items: SelectItem[], callback: (item: SelectItem) => void): void {
...
function renderSelectItems(items: SelectItem[]): void {
...
function selectModalChoice(index: number): void {
  const list = document.getElementById('select-modal-list');
...
// ==================== CONFETTI EFFECT ====================
function launchConfetti(duration: number = 2000): void {
...
// ==================== EXPORT FUNCTIONS ====================
function exportarEdital(formato: string): void {
  window.open(`/api/edital/exportar?formato=${formato}`, '_blank');
}

function exportarCiclo(formato: string): void {
