...
async function initProgress() {
  ...
  // O PDF.js embutido carrega a URL de `file=` diretamente (com range requests
  // nativos). O navegador NÃO anexa o header Authorization nesse fetch, então
  // passamos o token de acesso via query (?token=), aceito pelo backend em
  // get_user_id (header OU query). Evita a fragilidade de blob URLs entre
  // janela/iframe (que resultava em "0 páginas").
  const token = localStorage.getItem('auth_token');
  let pdfEndpoint = `${location.origin}/pdf/${encodePath(path)}`;
  if (token) pdfEndpoint += `?token=${encodeURIComponent(token)}`;
...
async function loadSidePanelQuestions() {
  ...
  try {
    ...
    // Servir cada questão EMBARALHADA. Modo NÃO-determinístico: gera uma seed
    // aleatória por questão ao servir; o backend devolve essa seed (campo `seed`),
    // que é gravada em data-seed e reenviada no /responder para validar na ordem
    // exibida. Fallback: mantém a ordem original se o fetch falhar.
    const embaralhadas = await Promise.all(shuffled.map(async (q) => {
      try {
        const seed = Math.floor(Math.random() * 2147483647);
        const emb = await fetch(`/api/questoes/${q.id}?embaralhar=true&seed=${seed}`).then(r => r.ok ? r.json() : null);
...
function renderSidePanelQuestions(questoes) {
  ...
  body.innerHTML = questoes.map((q, idx) => {
    ...
    return `<div class="sp-question" data-id="${q.id}" data-correct="${q.resposta_correta}" data-embaralhada="${q.embaralhada ? '1' : ''}" data-seed="${q.seed ?? ''}" data-start="${Date.now()}">
      <div class="sp-meta">
...
async function selectSideAlt(el) {
  ...
  const seed = question.dataset.seed !== '' ? parseInt(question.dataset.seed, 10) : null;
  ...
  const res = await fetch(`/api/questoes/${qId}/responder`, {
    method: 'POST',
    headers: { 'Content-Type': 'application/json' },
    body: JSON.stringify({ resposta: letter, tempo_segundos: tempoSegundos, embaralhada, seed })
  }).then(r => r.json());
      ...
      list.innerHTML = notes.map(n => `
        <div style="background:#1e1e2e;border-radius:8px;padding:10px;margin-bottom:8px;position:relative;">
          ...
          <div style="display:flex;justify-content:space-between;margin-top:6px;">
            ...
            <button onclick="deleteNote(${n.id})" style="background:none;border:none;color:var(--red,#f38ba8);opacity:0.75;cursor:pointer;font-size:0.75rem;" aria-label="Excluir nota">🗑</button>
          </div>
        </div>
      `).join('');
    }
